# 01. AutoVibe 30분 퀵스타트

> **목표**: 이 가이드를 따라 30분 안에 Phase 0 기반 인프라를 완료합니다.
> AutoVibe를 처음 접하는 분을 위한 단계별 타임라인입니다.

---

## 전체 흐름

```mermaid
flowchart LR
    A["0~2분<br/>저장소 준비<br/>문서 복사"] --> B["2~5분<br/>Claude Code 시작<br/>Phase 0 대화"]
    B --> C["5~15분<br/>Claude 질문 4가지<br/>답변하기"]
    C --> D["15~25분<br/>Phase 0 구현<br/>자동 생성 대기"]
    D --> E["25~30분<br/>완료 검증<br/>다음 Phase"]

    style A fill:#2d6a4f,color:#fff
    style C fill:#1d3557,color:#fff
    style E fill:#457b9d,color:#fff
```

---

## AutoVibe 생태계란

AutoVibe는 **3개의 독립 플랫폼**을 하나로 통합하는 AI 개발 생태계입니다.

| 플랫폼 | 역할 | 비유 |
|--------|------|------|
| **Claude Code** | AI 런타임 엔진 — 에이전트가 생각하고 코드를 만듦 | 개발팀의 두뇌 |
| **gstack** | Fast Headless Browser — 만든 것을 눈으로 보고 테스트 | 개발팀의 눈 |
| **bkit** | Vibecoding Kit — 설계 대비 품질을 측정하고 개선 | 개발팀의 체크리스트 |

이 3축을 `/av {자연어}` 하나로 제어합니다. 30분 안에 기반을 구축해 봅시다.

---

## 시작 전 확인 (2분)

```bash
claude --version    # v2.1.71 이상 필요
git --version       # git 2.x 이상
```

Claude Code 내에서 플러그인 확인:
```
/bkit          # bkit 메뉴 확인
/gstack        # gstack 메뉴 확인
```
두 메뉴가 모두 나타나면 준비 완료. 안 나타나면 [00-WSL-환경설정.md](00-WSL-환경설정.md) 참고.

---

## 0:00~2:00 -- 저장소 준비

```bash
# AutoVibe 저장소 클론
git clone https://github.com/s99606931/autovibe.git /tmp/autovibe

# 내 프로젝트로 이동
cd my-project

# AutoVibe 문서 복사
mkdir -p docs/autovibe
cp -r /tmp/autovibe/docs/* docs/autovibe/
cp -r /tmp/autovibe/guides docs/autovibe/guides
```

---

## 2:00~5:00 -- Claude Code 시작

```bash
cd my-project
claude
```

Claude에게 입력:
```
AutoVibe 생태계를 구축하고 싶어. Phase 0부터 시작해줘.
docs/autovibe/design/av-ecosystem-design-spec.md 를 참고해서.
```

---

## 5:00~15:00 -- Claude의 질문에 답하기

```mermaid
sequenceDiagram
    participant U as 사용자
    participant C as Claude

    C->>U: 프로젝트 이름은 무엇인가요?
    U->>C: my-saas
    C->>U: 기술 스택은 무엇인가요?
    U->>C: NestJS + Next.js
    C->>U: 주요 도메인 그룹은?
    U->>C: user, order, payment
    C->>U: 소스 루트 경로는?
    U->>C: src (기본값 Enter)
    C->>C: Phase 0 구현 시작...
```

| 질문 | 예시 답변 | 비고 |
|------|---------|------|
| "프로젝트 이름은?" | `my-saas` | 영문 소문자, 하이픈 가능 |
| "기술 스택은?" | `NestJS + Next.js` | 백엔드 + 프론트엔드 |
| "주요 도메인은?" | `user, order, payment` | 콤마 구분 |
| "소스 루트 경로는?" | `src` | 기본값 Enter 가능 |

---

## 15:00~25:00 -- Phase 0 구현 완료 대기

Claude가 자동으로 생성하는 것들:

```mermaid
graph TD
    subgraph ".claude/ 디렉토리"
        SK["skills/ — 스킬 파일"]
        AG["agents/ — 에이전트 파일"]
        RU["rules/ — 규칙 파일"]
        HK["hooks/ — 훅 스크립트"]
        RE["registry/<br/>components.json"]
        AM["agent-memory/ — 학습 기록"]
    end

    CL["CLAUDE.md<br/>AutoVibe 섹션 추가"]
    SE["settings.json<br/>빈 훅 설정"]

    RE --- CL
    RE --- SE
```

---

## 25:00~30:00 -- 완료 검증

```bash
ls -la .claude/                              # 6개 하위 디렉토리
cat .claude/registry/components.json         # 기본 구조
grep -A 5 "AutoVibe" CLAUDE.md               # AutoVibe 섹션
```

기대 결과:
- `.claude/` 하위 6개 디렉토리 존재
- `components.json` 기본 구조 (`_meta`, `rules`, `agents`, `skills`, `hooks`)
- `CLAUDE.md`에 AutoVibe 관련 섹션

---

## Phase 0 이후 -- 다음 단계

```mermaid
flowchart TD
    P0["✅ Phase 0 완료<br/>기반 인프라 (이 가이드)"]
    P1["Phase 1 — 10분<br/>Base Rules 4종"]
    P2["Phase 2 — 20분<br/>Base Agents 8종"]
    P3["Phase 3 — 20분<br/>Meta Skills 6종"]
    P4["Phase 4 — 20분<br/>Core Skills 10종"]
    P5["Phase 5 — 10분<br/>Hooks 5종"]
    P6["Phase 6 — 무제한<br/>도메인 확장"]

    P0 --> P1 --> P2 --> P3 --> P4 --> P5 --> P6
    P6 -->|"새 도메인 필요 시"| P6

    style P0 fill:#2d6a4f,color:#fff
    style P6 fill:#457b9d,color:#fff
```

**총 기반 구축: 약 2시간**

---

## 문제 발생 시

| 증상 | 해결 |
|------|------|
| Claude가 AutoVibe를 모름 | `docs/autovibe/design/av-ecosystem-design-spec.md 파일을 읽어줘` |
| Phase 도중 멈춤 | `/pdca status` → 해당 Phase 재시작 |
| bkit을 찾을 수 없음 | Claude Code 재시작 또는 `/plugin install bkit` |

---

**다음**: [02-설계-철학.md](02-설계-철학.md) -- AutoVibe가 왜 이렇게 설계되었는지
