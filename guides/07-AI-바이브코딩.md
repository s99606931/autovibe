...
일상적인 사용은 `/av {자연어}`로 충분하지만, 세부 제어가 필요할 때:

| 목적 | 명령어 |
|------|--------|
| 특정 URL 테스트 | `/gstack navigate https://example.com` |
| 스크린샷 캡처 | `/gstack screenshot` |
| PDCA 문서 작성 | `/bkit:pdca plan {feature}` |
| Gap 분석 | `/bkit:pdca analyze {feature}` |
| 성능 벤치마크 | `/benchmark` |
| 카나리 배포 | `/canary` |
